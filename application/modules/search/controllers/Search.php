...
class Search extends CI_Controller {
	...
	public function index(){
		$data['title'] = 'QShoes | Pencarian';

		if ($this->session->userdata('username') != '') {
			$data['id_user'] = $this->session->userdata('id_user');
			$data['username'] = $this->session->userdata('username');

			$resU = $this->SearchModel->getUserData($data['id_user']);
			$resT = $this->SearchModel->getTokoData($data['id_user']);
			...
			$no = 0;
			$newOrder = $this->SearchModel->getNewOrder($this->session->userdata('id_toko'));
			$tUnread = 0;
			$unread = $this->SearchModel->getUnreadMsg($this->session->userdata('id_toko'));
			...
			$myOrder = $this->SearchModel->getMyOrder($this->session->userdata('id_user'));
